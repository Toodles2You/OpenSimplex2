...
static const float GRADIENTS_2D[] = {
    #include GRADIENTS_2D_SRC
};
static const float GRADIENTS_3D[] = {
    #include GRADIENTS_3D_SRC
};
static const float GRADIENTS_4D[] = {
    #include GRADIENTS_4D_SRC
};
...
static float noise2_UnskewedBase(long seed, double xs, double ys) {
    ...
    if (a0 > 0) {
        value = (a0 * a0) * (a0 * a0) * noise2_grad(seed, xsbp, ysbp, dx0, dy0);
    }
    ...
    if (a1 > 0) {
        ...
        value += (a1 * a1) * (a1 * a1) * noise2_grad(seed, xsbp + PRIME_X, ysbp + PRIME_Y, dx1, dy1);
    }
    ...
    if (dy0 > dx0) {
        ...
        if (a2 > 0) {
            value += (a2 * a2) * (a2 * a2) * noise2_grad(seed, xsbp, ysbp + PRIME_Y, dx2, dy2);
        }
    }
    else
    {
        ...
        float a2 = RSQUARED_2D - dx2 * dx2 - dy2 * dy2;
        if (a2 > 0) {
            value += (a2 * a2) * (a2 * a2) * noise2_grad(seed, xsbp + PRIME_X, ysbp, dx2, dy2);
        }
    }
...
static float noise3_UnrotatedBase(long seed, double xr, double yr, double zr) {
    ...
    for (int l = 0; ; l++) {
        ...
        if (a > 0) {
            value += (a * a) * (a * a) * noise3_grad(seed, xrbp, yrbp, zrbp, xri, yri, zri);
        }
        ...
        if (ax0 >= ay0 && ax0 >= az0) {
            ...
            if (b > 1) {
                b -= 1;
                value += (b * b) * (b * b) * noise3_grad(seed, xrbp - xNSign * PRIME_X, yrbp, zrbp, xri + xNSign, yri, zri);
            }
        }
            ...
            if (b > 1) {
                b -= 1;
                value += (b * b) * (b * b) * noise3_grad(seed, xrbp, yrbp - yNSign * PRIME_Y, zrbp, xri, yri + yNSign, zri);
            }
        }
        else
        {
            ...
            if (b > 1) {
                b -= 1;
                value += (b * b) * (b * b) * noise3_grad(seed, xrbp, yrbp, zrbp - zNSign * PRIME_Z, xri, yri, zri + zNSign);
            }
        }
...
static float noise4_UnskewedBase(long seed, double xs, double ys, double zs, double ws) {
    ...
    for (int i = 0; ; i++) {
        ...
        if (a < RSQUARED_4D) {
            a -= RSQUARED_4D;
            a *= a;
            value += a * a * noise4_grad(seed, xsvp, ysvp, zsvp, wsvp, dx, dy, dz, dw);
        }
